...
def odds(p): return "—" if not p else f"{min(999, 1/p):.2f}"
def usd(v):
    if v is None: return "—"
...
# 注目の動き
rows = [(ev, o) for ev in D["events"] for o in ev["outcomes"] if o["prob"] is not None]
...
def cat_table(evs):
    out = [f'<table cellspacing="0" cellpadding="0" width="100%"><tr><th {TH}>マーケット</th><th {TH}>首位の選択肢</th><th {THR}>確率</th><th {THR}>オッズ</th><th {THR}>24h</th><th {THR}>7日</th><th {THR}>出来高</th></tr>']
    for ev in evs:
        outs = sorted(ev["outcomes"], key=lambda o: -(o["prob"] or 0))
        top = outs[0]
        second = ""
        if len(outs) > 1 and outs[1]["prob"] is not None and (ev.get("sum_to_one", True)):
            second = f'<br><span style="color:#777;font-size:12px">2位 {e(oname(outs[1], ev))} {pct(outs[1]["prob"])}</span>'
        vol = f'M{ev["volume"]:,}' if ev["source"] == "manifold" else usd(ev["volume"])
        link = f' <a href="{e(ev["url"])}" style="color:#2b3f7a;text-decoration:none;font-size:11px">↗</a>'
        out.append(f'<tr><td {TD}>{e(ev["title_ja"])}<br><span style="color:#999;font-size:11px">{SRC[ev["source"]]}</span>{link}</td><td {TD}>{e(oname(top, ev))}{second}</td><td {TDR}><b>{pct(top["prob"])}</b></td><td {TDR}>{odds(top["prob"])}</td><td {TDR}>{delta(top["prob"], top.get("prob_24h"))}</td><td {TDR}>{delta(top["prob"], top.get("prob_7d"))}</td><td {TDR} style="color:#888">{vol}</td></tr>')
    return "".join(out) + "</table>"
...
<p style="font-size:12px;color:#666;background:#f3f4f7;padding:8px 10px;border-left:3px solid #2b3f7a;margin:12px 0 0">確率＝各市場のYES価格、オッズ＝欧州式（1÷確率）。変化幅は確率のポイント差で、<span style="color:#c8302a">上昇＝赤</span>・<span style="color:#1d7a4f">下落＝緑</span>。Manifoldはプレイマネーのため参考値。</p>''']
parts.append(f'<h2 {H2}>24時間で動いた賭け</h2>' + mover_table(mv24, "prob_24h"))
parts.append(f'<h2 {H2}>7日間で動いた賭け</h2>' + mover_table(mv7, "prob_7d"))
groups = {}
for ev in D["events"]: groups.setdefault(ev["cat"], []).append(ev)
...
big = max((x for x in D["events"] if x["source"] != "manifold" and not x["title_ja"].startswith("【関連】")), key=lambda x: x["volume"] or 0, default=None)
if big:
